...
export default function ProfileViewsChart({ data, thisWeek, lastWeek }: ProfileViewsChartProps) {
  ...
  const trendLabel =
    ...
    : "Same as last week";
  const trendColor = trend > 0 ? "#7BA84A" : trend < 0 ? "#E26B5E" : "#5C5C7A";

  return (
    <section className="rounded-xl p-5" style={{ background: "white", border: "1px solid #E8E8F4" }}>
      <div className="flex items-start justify-between mb-4">
        <div>
          <h2 className="text-sm font-semibold" style={{ color: "#1A1A2E" }}>Profile views</h2>
          <p className="text-xs" style={{ color: "#5C5C7A" }}>Last 7 days</p>
        </div>
        <div className="text-right">
          <p className="text-2xl font-light leading-none" style={{ color: "#1A1A2E" }}>{thisWeek}</p>
        ...
        {data.map(({ day, views }) => (
          <div key={day} className="flex-1 flex flex-col items-center gap-1 h-full justify-end">
            <div
              ...
              style={{ height: `${Math.max((views / max) * 96, 4)}px`, background: "#6B6DE8" }}
            />
          </div>
        ))}
      </div>
      <div className="flex gap-1.5">
        {data.map(({ day, views }) => (
          <div key={day} className="flex-1 text-center">
            <span className="text-xs leading-none block" style={{ color: "#5C5C7A" }}>{day}</span>
            <span className="text-xs font-semibold leading-none block" style={{ color: "#5C5C7A" }}>{views}</span>
          </div>
        ))}
      </div>
    </section>
